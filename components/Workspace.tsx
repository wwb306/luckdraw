...
import { Settings, Play, Square, RefreshCcw, Download, Gift, Users, ArrowLeft } from 'lucide-react';
import ConfigPanel from './ConfigPanel';
import Sidebar from './Sidebar';
import { exportWinners } from '../utils/excelHelper';
import { Participant, Prize, Project, AppState } from '../types';

interface WorkspaceProps {
  project: Project;
  onSave: (data: Partial<AppState>) => void;
  onExit: () => void;
}

export function Workspace({ project, onSave, onExit }: WorkspaceProps) {
  const [participants, setParticipants] = useState<Participant[]>(project.data.participants);
  const [prizes, setPrizes] = useState<Prize[]>(project.data.prizes);
  ...
  useEffect(() => {
    onSave({ participants, prizes });
  }, [participants, prizes, onSave]);
